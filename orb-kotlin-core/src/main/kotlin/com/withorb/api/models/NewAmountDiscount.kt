...
private constructor(
    ...
    class Builder internal constructor() {
        ...
        internal fun from(newAmountDiscount: NewAmountDiscount) = apply {
            ...
            appliesToAll = newAmountDiscount.appliesToAll
            appliesToItemIds = newAmountDiscount.appliesToItemIds.map { it.toMutableList() }
            appliesToPriceIds = newAmountDiscount.appliesToPriceIds.map { it.toMutableList() }
            currency = newAmountDiscount.currency
            filters = newAmountDiscount.filters.map { it.toMutableList() }
            isInvoiceLevel = newAmountDiscount.isInvoiceLevel
            priceType = newAmountDiscount.priceType
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return /* spotless:off */ other is NewAmountDiscount && adjustmentType == other.adjustmentType && amountDiscount == other.amountDiscount && appliesToAll == other.appliesToAll && appliesToItemIds == other.appliesToItemIds && appliesToPriceIds == other.appliesToPriceIds && currency == other.currency && filters == other.filters && isInvoiceLevel == other.isInvoiceLevel && priceType == other.priceType && additionalProperties == other.additionalProperties /* spotless:on */
    }

    /* spotless:off */
    private val hashCode: Int by lazy { Objects.hash(adjustmentType, amountDiscount, appliesToAll, appliesToItemIds, appliesToPriceIds, currency, filters, isInvoiceLevel, priceType, additionalProperties) }
    /* spotless:on */

    override fun hashCode(): Int = hashCode

    override fun toString() =
        "NewAmountDiscount{adjustmentType=$adjustmentType, amountDiscount=$amountDiscount, appliesToAll=$appliesToAll, appliesToItemIds=$appliesToItemIds, appliesToPriceIds=$appliesToPriceIds, currency=$currency, filters=$filters, isInvoiceLevel=$isInvoiceLevel, priceType=$priceType, additionalProperties=$additionalProperties}"
}

...
private constructor(
     ...
     */
    fun appliesToAll(): AppliesToAll? = appliesToAll.getNullable("applies_to_all")
     ...
     *   responded with an unexpected value).
     */
    fun appliesToItemIds(): List<String>? = appliesToItemIds.getNullable("applies_to_item_ids")
     ...
     *   responded with an unexpected value).
     */
    fun isInvoiceLevel(): Boolean? = isInvoiceLevel.getNullable("is_invoice_level")
     ...
     *   responded with an unexpected value).
     */
    fun priceType(): PriceType? = priceType.getNullable("price_type")
    ...
    class Builder internal constructor() {
        ...
        internal fun from(newUsageDiscount: NewUsageDiscount) = apply {
            ...
            appliesToAll = newUsageDiscount.appliesToAll
            appliesToItemIds = newUsageDiscount.appliesToItemIds.map { it.toMutableList() }
            appliesToPriceIds = newUsageDiscount.appliesToPriceIds.map { it.toMutableList() }
            currency = newUsageDiscount.currency
            filters = newUsageDiscount.filters.map { it.toMutableList() }
            isInvoiceLevel = newUsageDiscount.isInvoiceLevel
            priceType = newUsageDiscount.priceType
    ...
    internal fun validity(): Int =
        (adjustmentType.asKnown()?.validity() ?: 0) +
            ...
            (appliesToAll.asKnown()?.validity() ?: 0) +
            (appliesToItemIds.asKnown()?.size ?: 0) +
    ...
    class AppliesToAll @JsonCreator private constructor(private val value: JsonField<Boolean>) :
        ...
        companion object {
            ...
            fun of(value: Boolean) = AppliesToAll(JsonField.of(value))
        }
        ...
        fun asBoolean(): Boolean =
            _value().asBoolean() ?: throw OrbInvalidDataException("Value is not a Boolean")
    ...
    class PriceType @JsonCreator private constructor(private val value: JsonField<String>) : Enum {
        ...
        companion object {
            ...
            val FIXED_IN_ADVANCE = of("fixed_in_advance")

            val FIXED_IN_ARREARS = of("fixed_in_arrears")

            val FIXED = of("fixed")

            val IN_ARREARS = of("in_arrears")
    ...
    override fun equals(other: Any?): Boolean {
        ...
        return /* spotless:off */ other is NewUsageDiscount && adjustmentType == other.adjustmentType && usageDiscount == other.usageDiscount && appliesToAll == other.appliesToAll && appliesToItemIds == other.appliesToItemIds && appliesToPriceIds == other.appliesToPriceIds && currency == other.currency && filters == other.filters && isInvoiceLevel == other.isInvoiceLevel && priceType == other.priceType && additionalProperties == other.additionalProperties /* spotless:on */
    }

    /* spotless:off */
    private val hashCode: Int by lazy { Objects.hash(adjustmentType, usageDiscount, appliesToAll, appliesToItemIds, appliesToPriceIds, currency, filters, isInvoiceLevel, priceType, additionalProperties) }
    /* spotless:on */

    override fun hashCode(): Int = hashCode

    override fun toString() =
        "NewUsageDiscount{adjustmentType=$adjustmentType, usageDiscount=$usageDiscount, appliesToAll=$appliesToAll, appliesToItemIds=$appliesToItemIds, appliesToPriceIds=$appliesToPriceIds, currency=$currency, filters=$filters, isInvoiceLevel=$isInvoiceLevel, priceType=$priceType, additionalProperties=$additionalProperties}"
}

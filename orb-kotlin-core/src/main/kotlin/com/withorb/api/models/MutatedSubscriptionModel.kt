...
@NoAutoDetect
class MutatedSubscriptionModel
@JsonCreator
private constructor(
    ...
    fun redeemedCoupon(): CouponRedemptionModel? = redeemedCoupon.getNullable("redeemed_coupon")
    ...
    fun validate(): MutatedSubscriptionModel = apply {
        if (validated) {
            return@apply
        }
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [MutatedSubscriptionModel].
         *
    ...
    /** A builder for [MutatedSubscriptionModel]. */
    class Builder internal constructor() {
        ...
        internal fun from(mutatedSubscriptionModel: MutatedSubscriptionModel) = apply {
            id = mutatedSubscriptionModel.id
            activePlanPhaseOrder = mutatedSubscriptionModel.activePlanPhaseOrder
            adjustmentIntervals =
                mutatedSubscriptionModel.adjustmentIntervals.map { it.toMutableList() }
            autoCollection = mutatedSubscriptionModel.autoCollection
            billingCycleAnchorConfiguration =
                mutatedSubscriptionModel.billingCycleAnchorConfiguration
            billingCycleDay = mutatedSubscriptionModel.billingCycleDay
            createdAt = mutatedSubscriptionModel.createdAt
            currentBillingPeriodEndDate = mutatedSubscriptionModel.currentBillingPeriodEndDate
            currentBillingPeriodStartDate = mutatedSubscriptionModel.currentBillingPeriodStartDate
            customer = mutatedSubscriptionModel.customer
            defaultInvoiceMemo = mutatedSubscriptionModel.defaultInvoiceMemo
            discountIntervals =
                mutatedSubscriptionModel.discountIntervals.map { it.toMutableList() }
            endDate = mutatedSubscriptionModel.endDate
            fixedFeeQuantitySchedule =
                mutatedSubscriptionModel.fixedFeeQuantitySchedule.map { it.toMutableList() }
            invoicingThreshold = mutatedSubscriptionModel.invoicingThreshold
            maximumIntervals = mutatedSubscriptionModel.maximumIntervals.map { it.toMutableList() }
            metadata = mutatedSubscriptionModel.metadata
            minimumIntervals = mutatedSubscriptionModel.minimumIntervals.map { it.toMutableList() }
            netTerms = mutatedSubscriptionModel.netTerms
            plan = mutatedSubscriptionModel.plan
            priceIntervals = mutatedSubscriptionModel.priceIntervals.map { it.toMutableList() }
            redeemedCoupon = mutatedSubscriptionModel.redeemedCoupon
            startDate = mutatedSubscriptionModel.startDate
            status = mutatedSubscriptionModel.status
            trialInfo = mutatedSubscriptionModel.trialInfo
            additionalProperties = mutatedSubscriptionModel.additionalProperties.toMutableMap()
        }
        ...
        fun build(): MutatedSubscriptionModel =
    ...
    private constructor(
        ...
        fun amountDiscountIntervalModel(): AmountDiscountIntervalModel? =
            amountDiscountIntervalModel

        fun percentageDiscountIntervalModel(): PercentageDiscountIntervalModel? =
            percentageDiscountIntervalModel

        fun usageDiscountIntervalModel(): UsageDiscountIntervalModel? = usageDiscountIntervalModel

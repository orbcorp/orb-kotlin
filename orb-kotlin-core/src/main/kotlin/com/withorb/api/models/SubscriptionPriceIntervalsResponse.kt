...
@NoAutoDetect
class SubscriptionPriceIntervalsResponse @JsonCreator private constructor(
    ...
    fun validate(): SubscriptionPriceIntervalsResponse =
        apply {
            if (validated) {
              return@apply
            }
    ...
    /** A builder for [SubscriptionPriceIntervalsResponse]. */
    class Builder internal constructor() {
        ...
        internal fun from(subscriptionPriceIntervalsResponse: SubscriptionPriceIntervalsResponse) =
            apply {
                id = subscriptionPriceIntervalsResponse.id
                activePlanPhaseOrder = subscriptionPriceIntervalsResponse.activePlanPhaseOrder
                adjustmentIntervals = subscriptionPriceIntervalsResponse.adjustmentIntervals.map { it.toMutableList() }
                autoCollection = subscriptionPriceIntervalsResponse.autoCollection
                billingCycleAnchorConfiguration = subscriptionPriceIntervalsResponse.billingCycleAnchorConfiguration
                billingCycleDay = subscriptionPriceIntervalsResponse.billingCycleDay
                createdAt = subscriptionPriceIntervalsResponse.createdAt
                currentBillingPeriodEndDate = subscriptionPriceIntervalsResponse.currentBillingPeriodEndDate
                currentBillingPeriodStartDate = subscriptionPriceIntervalsResponse.currentBillingPeriodStartDate
                customer = subscriptionPriceIntervalsResponse.customer
                defaultInvoiceMemo = subscriptionPriceIntervalsResponse.defaultInvoiceMemo
                discountIntervals = subscriptionPriceIntervalsResponse.discountIntervals.map { it.toMutableList() }
                endDate = subscriptionPriceIntervalsResponse.endDate
                fixedFeeQuantitySchedule = subscriptionPriceIntervalsResponse.fixedFeeQuantitySchedule.map { it.toMutableList() }
                invoicingThreshold = subscriptionPriceIntervalsResponse.invoicingThreshold
                maximumIntervals = subscriptionPriceIntervalsResponse.maximumIntervals.map { it.toMutableList() }
                metadata = subscriptionPriceIntervalsResponse.metadata
                minimumIntervals = subscriptionPriceIntervalsResponse.minimumIntervals.map { it.toMutableList() }
                netTerms = subscriptionPriceIntervalsResponse.netTerms
                plan = subscriptionPriceIntervalsResponse.plan
                priceIntervals = subscriptionPriceIntervalsResponse.priceIntervals.map { it.toMutableList() }
                redeemedCoupon = subscriptionPriceIntervalsResponse.redeemedCoupon
                startDate = subscriptionPriceIntervalsResponse.startDate
                status = subscriptionPriceIntervalsResponse.status
                trialInfo = subscriptionPriceIntervalsResponse.trialInfo
                additionalProperties = subscriptionPriceIntervalsResponse.additionalProperties.toMutableMap()
            }

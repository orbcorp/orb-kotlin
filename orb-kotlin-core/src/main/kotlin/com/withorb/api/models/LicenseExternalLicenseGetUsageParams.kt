 ...
 *
 * Resolves the currently active license with this external ID, i.e. the license whose active window
 * contains the current time. An external license ID can map to multiple licenses over time, so if
 * none is currently active (deactivated, window ended, or not yet started) this returns a 404. To
 * fetch usage for a license regardless of active state, use the endpoint that takes Orb's internal
 * license ID.
 *

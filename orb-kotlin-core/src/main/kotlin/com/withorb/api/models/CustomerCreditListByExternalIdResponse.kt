...
private constructor(
     ...
     * allocation), `top_up` (an automatic balance-threshold top-up), `commitment` (a subscription
     * commitment true-up rolled forward as credit), or `manual` (a manual credit ledger increment,
     * including credits voided or expired off another block).
     *
     * @throws OrbInvalidDataException if the JSON field has an unexpected type or is unexpectedly
     *   missing or null (e.g. if the server responded with an unexpected value).
     */
     ...
     *   responded with an unexpected value).
     */
    fun creditAllocation(): CreditAllocation? = creditAllocation.getNullable("credit_allocation")

    /**
     * The subscription commitment whose true-up rolled forward into this credit block. Present only
     * when `credit_block_source` is `commitment`.
     *
     * @throws OrbInvalidDataException if the JSON field has an unexpected type (e.g. if the server
     *   responded with an unexpected value).
     */
    fun creditCommitment(): CreditCommitment? = creditCommitment.getNullable("credit_commitment")
    ...
    class Builder internal constructor() {
        ...
        internal fun from(
            ...
            creditBlockSource = customerCreditListByExternalIdResponse.creditBlockSource
            ...
            creditAllocation = customerCreditListByExternalIdResponse.creditAllocation
            creditCommitment = customerCreditListByExternalIdResponse.creditCommitment
         ...
         *
         * You should usually call [Builder.creditCommitment] with a well-typed [CreditCommitment]
         * value instead. This method is primarily for setting the field to an undocumented or not
         * yet supported value.
         */
    ...
    private constructor(
         ...
         */
        internal fun validity(): Int =
            (if (id.asKnown() == null) 0 else 1) + (if (subscriptionId.asKnown() == null) 0 else 1)
        ...
        private val hashCode: Int by lazy { Objects.hash(id, subscriptionId, additionalProperties) }
    ...
    override fun toString() =
        "CustomerCreditListByExternalIdResponse{id=$id, balance=$balance, creditBlockSource=$creditBlockSource, effectiveDate=$effectiveDate, expiryDate=$expiryDate, filters=$filters, maximumInitialBalance=$maximumInitialBalance, metadata=$metadata, perUnitCostBasis=$perUnitCostBasis, status=$status, creditAllocation=$creditAllocation, creditCommitment=$creditCommitment, additionalProperties=$additionalProperties}"
}

...
private constructor(
    ...
    fun validate(): SubscriptionModel = apply {
        if (validated) {
            return@apply
        }
    ...
    companion object {
        ...
        /**
         * Returns a mutable builder for constructing an instance of [SubscriptionModel].
         *
    ...
    /** A builder for [SubscriptionModel]. */
    class Builder internal constructor() {
        ...
        internal fun from(subscriptionModel: SubscriptionModel) = apply {
            id = subscriptionModel.id
            activePlanPhaseOrder = subscriptionModel.activePlanPhaseOrder
            adjustmentIntervals = subscriptionModel.adjustmentIntervals.map { it.toMutableList() }
            autoCollection = subscriptionModel.autoCollection
            billingCycleAnchorConfiguration = subscriptionModel.billingCycleAnchorConfiguration
            billingCycleDay = subscriptionModel.billingCycleDay
            createdAt = subscriptionModel.createdAt
            currentBillingPeriodEndDate = subscriptionModel.currentBillingPeriodEndDate
            currentBillingPeriodStartDate = subscriptionModel.currentBillingPeriodStartDate
            customer = subscriptionModel.customer
            defaultInvoiceMemo = subscriptionModel.defaultInvoiceMemo
            discountIntervals = subscriptionModel.discountIntervals.map { it.toMutableList() }
            endDate = subscriptionModel.endDate
            fixedFeeQuantitySchedule =
                subscriptionModel.fixedFeeQuantitySchedule.map { it.toMutableList() }
            invoicingThreshold = subscriptionModel.invoicingThreshold
            maximumIntervals = subscriptionModel.maximumIntervals.map { it.toMutableList() }
            metadata = subscriptionModel.metadata
            minimumIntervals = subscriptionModel.minimumIntervals.map { it.toMutableList() }
            netTerms = subscriptionModel.netTerms
            plan = subscriptionModel.plan
            priceIntervals = subscriptionModel.priceIntervals.map { it.toMutableList() }
            redeemedCoupon = subscriptionModel.redeemedCoupon
            startDate = subscriptionModel.startDate
            status = subscriptionModel.status
            trialInfo = subscriptionModel.trialInfo
            additionalProperties = subscriptionModel.additionalProperties.toMutableMap()
        }

...
interface SubscriptionServiceAsync {
    ...
    /** @see create */
    suspend fun create(requestOptions: RequestOptions): MutatedSubscription =
    ...
    /** @see fetch */
    suspend fun fetch(subscriptionId: String, requestOptions: RequestOptions): Subscription =
     ...
     * ## Trials
     *
     * Price intervals inside a subscription's trial period cannot be modified through this
     * endpoint. The restriction applies to the whole price interval, not only to its dates, so a
     * request that edits `filter` or `usage_customer_ids` and leaves the dates unset is rejected in
     * the same way. Trial boundaries are moved with the
     * [update trial](/api-reference/subscription/update-trial) endpoint. A price also cannot be
     * added with a `start_date` that falls before the trial ends.
     *
     * Each usage price in the trial has a second price interval that begins when the trial ends and
     * is not subject to these restrictions. Editing that interval applies the change to all usage
     * from the trial end onward.
     *
     ...
     * cancellation, and the subscription continues on the new plan. Scheduling a plan change after
     * the subscription's end date returns a validation error instead.
     *
     ...
     * validation error.
     *
     * Note: uncancellation is a lossy operation. Price and adjustment intervals that were cut short
     * by the cancellation are extended to infinity (original end dates are lost), and future
     * intervals or phases scheduled after the cancellation time are permanently deleted. For
     * complex subscriptions with phases or scheduled plan changes, consider creating a new plan
     * change instead of uncancelling.
     *
     * If the scheduled cancellation already produced invoices or credit notes (for example, a
     * proration refund credit note for an in-advance fee), uncancelling voids and reissues them as
     * needed to match the subscription's new state.
     */
     ...
     * When called, all upcoming plan changes will be unscheduled; it is not possible to unschedule
     * a single plan change if multiple are scheduled.
     *
     ...
     * Prices and adjustments on the current plan that were scheduled to end at the plan change time
     * are extended to infinity (original end dates are lost), and anything scheduled to start after
     * the plan change time is permanently deleted. Coupons redeemed as part of an unscheduled plan
     * change are released and can be redeemed again.
     *
    ...
    interface WithRawResponse {
        ...
        /** @see create */
        @MustBeClosed
        suspend fun create(requestOptions: RequestOptions): HttpResponseFor<MutatedSubscription> =
            create(SubscriptionCreateParams.none(), requestOptions)

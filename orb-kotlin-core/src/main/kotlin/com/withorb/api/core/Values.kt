...
sealed class JsonField<out T : Any> {
    ...
    fun asStringOrThrow(): String = asString() ?: throw OrbInvalidDataException("Value is not a string")
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw OrbInvalidDataException("`$name` is not set")
            is JsonNull -> throw OrbInvalidDataException("`$name` is null")
            else -> throw OrbInvalidDataException("`$name` is invalid, received $this")
        }
